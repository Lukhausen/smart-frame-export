...
import { Upload, Slider, Button, Space, message, Typography, Layout, Tooltip } from 'antd';
import {
  InboxOutlined,
  PlayCircleOutlined,
  PauseCircleOutlined,
  ...
  RightOutlined,
  CaretUpOutlined,
  StarOutlined, // For best frame buttons
  EyeOutlined,  // For local best frame
...
const PRIORITY_ANALYSIS_RADIUS = 10;
...
function App() {
  ...
  const [overallBestFrame, setOverallBestFrame] = useState(null); // Stores { frame, score, time }
  const [isUserInteracting, setIsUserInteracting] = useState(false);
  const [focusedHeatmapData, setFocusedHeatmapData] = useState([]); // Stores array of { frame, score, time, isCenter, isLocalBest, ... }
  ...
  const stableRefs = useRef({ /* ... same ... */ }).current;
  useEffect(() => { Object.assign(stableRefs, { frameRate, totalFrames, duration, isPlaying, overallBestFrame, videoSrc, currentTime, currentFrameNumber, isUserInteracting }); }, [frameRate, totalFrames, duration, isPlaying, overallBestFrame, videoSrc, currentTime, currentFrameNumber, isUserInteracting]);
  ...
  const dispatchToWorker = useCallback(async (imageData, frameNumberToAnalyze) => { /* ... same ... */
  ...
  // --- Utility Functions (timeToFrame, frameToTime - same) ---
  const timeToFrame = useCallback((time) => { /* ... same ... */
  ...
  const frameToTime = useCallback((frame) => { /* ... same ... */
  ...
  // --- Core Effects (videoSrc, totalFrames - same) ---
  useEffect(() => { /* for videoSrc changes */
  ...
  useEffect(() => { /* for duration/framerate changes */
  ...
  // --- Seek Video Element (same) ---
  const seekVideoElement = useCallback(async (videoElement, timeToSeek, isAnalysisSeek = false) => { /* ... same ... */
  ...
  // --- Analyze Single Frame (same) ---
  const analyzeSingleFrame = useCallback(async (frameNumberToAnalyze, isPriorityTask = false) => { /* ... same ... */
    ...
    if (analysisTaskController.analyzedScores.has(frameNumberToAnalyze) && !analysisTaskController.analyzedScores.get(frameNumberToAnalyze)?.error) { const existingData = analysisTaskController.analyzedScores.get(frameNumberToAnalyze); if (isPriorityTask && analysisTaskController.currentFocusedCenterFrame > 0 && Math.abs(frameNumberToAnalyze - analysisTaskController.currentFocusedCenterFrame) <= PRIORITY_ANALYSIS_RADIUS) { setFocusedHeatmapData(p => { const i = p.findIndex(d => d.frame === frameNumberToAnalyze); if (i > -1 && (!p[i].score || p[i].score !== existingData.score)) { const u = [...p]; u[i] = { ...u[i], score: existingData.score, time: existingData.time, error: false }; return u; } return p; }); } return existingData; }
    const time = frameToTime(frameNumberToAnalyze); let scoreData = null;
    try { await seekVideoElement(analysisVideoRef.current, time, true); if (analysisTaskController.isGloballyCancelled) return null; const actualTimeAfterSeek = analysisVideoRef.current.currentTime; if (!analysisCanvasRef.current) analysisCanvasRef.current = document.createElement('canvas'); if (analysisVideoRef.current.readyState < 2) { await new Promise(r => setTimeout(r, 50)); if (analysisVideoRef.current.readyState < 2 || analysisTaskController.isGloballyCancelled) return null; } if (analysisVideoRef.current.videoWidth === 0 || analysisVideoRef.current.videoHeight === 0) { throw new Error("Video dimensions zero"); } const canvas = analysisCanvasRef.current; const videoElement = analysisVideoRef.current; const ctx = canvas.getContext('2d', { willReadFrequently: true }); const analysisWidth = 160; const aspectRatio = videoElement.videoWidth / videoElement.videoHeight; const analysisHeight = Math.max(1, Math.floor(aspectRatio > 0 ? analysisWidth / aspectRatio : analysisWidth * (9/16) )); canvas.width = analysisWidth; canvas.height = analysisHeight; ctx.drawImage(videoElement, 0, 0, analysisWidth, analysisHeight); const imageData = ctx.getImageData(0, 0, analysisWidth, analysisHeight); const score = await dispatchToWorker(imageData, frameNumberToAnalyze); scoreData = { score, time: actualTimeAfterSeek, frame: frameNumberToAnalyze, error: false }; analysisTaskController.analyzedScores.set(frameNumberToAnalyze, scoreData); analysisTaskController.globalMinScore = Math.min(analysisTaskController.globalMinScore, score); analysisTaskController.globalMaxScore = Math.max(analysisTaskController.globalMaxScore, score); if (!stableRefs.overallBestFrame || score > stableRefs.overallBestFrame.score) { setOverallBestFrame(scoreData); } if (isPriorityTask && analysisTaskController.currentFocusedCenterFrame > 0 && Math.abs(frameNumberToAnalyze - analysisTaskController.currentFocusedCenterFrame) <= PRIORITY_ANALYSIS_RADIUS) { setFocusedHeatmapData(p => { const i = p.findIndex(d => d.frame === frameNumberToAnalyze); if (i > -1) { const u = [...p]; u[i] = { ...u[i], score: score, time: actualTimeAfterSeek, error: false }; return u; } return p; }); }
    } catch (error) { console.error(`Err F${frameNumberToAnalyze}:`, error.message); scoreData = { score: 0, time, frame: frameNumberToAnalyze, error: true }; analysisTaskController.analyzedScores.set(frameNumberToAnalyze, scoreData); if (isPriorityTask && analysisTaskController.currentFocusedCenterFrame > 0 && Math.abs(frameNumberToAnalyze - analysisTaskController.currentFocusedCenterFrame) <= PRIORITY_ANALYSIS_RADIUS) { setFocusedHeatmapData(p => { const i = p.findIndex(d => d.frame === frameNumberToAnalyze); if (i > -1) { const u = [...p]; u[i] = { ...u[i], score: null, error: true }; return u; } return p; }); } }
    return scoreData;
  }, [seekVideoElement, frameToTime, dispatchToWorker, stableRefs]);

  // --- Analysis Scheduling Logic (same parallel logic) ---
  const scheduleNextAnalysis = useCallback(() => { /* ... same parallel logic ... */
  ...
  // --- Main Analysis Lifecycle Effect (same) ---
  useEffect(() => { /* ... same ... */
    if (videoSrc && totalFrames > 0 && duration > 0) { if (analysisTaskController.nextScheduledAnalysisId) cancelAnimationFrame(analysisTaskController.nextScheduledAnalysisId); analysisTaskController.isGloballyCancelled = false; analysisTaskController.isRecursiveAnalysisPaused = false; analysisTaskController.isSeekingVideoLocked = false; analysisTaskController.currentSeekPromise = null; analysisTaskController.currentSeekerType = null; analysisTaskController.recursiveQueue = [{ startFrame: 1, endFrame: totalFrames, depth: 0 }]; analysisTaskController.priorityQueue = []; analysisTaskController.analyzedScores.clear(); analysisTaskController.globalMinScore = Infinity; analysisTaskController.globalMaxScore = -Infinity; analysisTaskController.currentFocusedCenterFrame = 0; analysisTaskController.activeAnalysisProcesses = 0; workerTaskCallbacks.current.forEach(t => t.reject(new Error("New video"))); workerTaskCallbacks.current.clear(); workerBusyStates.current.fill(false); setOverallBestFrame(null); setFocusedHeatmapData([]); setShowFocusedHeatmap(false); setHeatmapVersion(v => v + 1); scheduleNextAnalysis(); }
  ...
  useEffect(() => {
    if (showFocusedHeatmap && analysisTaskController.currentFocusedCenterFrame > 0 && stableRefs.totalFrames > 0) {
      const center = analysisTaskController.currentFocusedCenterFrame; const radius = PRIORITY_ANALYSIS_RADIUS; const currentFocusData = [];
      for (let i = -radius; i <= radius; i++) {
        const frame = center + i;
        if (frame >= 1 && frame <= stableRefs.totalFrames) {
          const sD = analysisTaskController.analyzedScores.get(frame);
          currentFocusData.push({ frame, score: sD ? sD.score : null, time: sD ? sD.time : null, isCenter: frame === center, error: sD ? !!sD.error : false, needsAnalysis: !sD || !!sD.error });
        }
      }
      let localBestScore = -Infinity; let localBestFrameData = null; // Store full data for local best
      currentFocusData.forEach(item => { if (item.score !== null && !item.error && item.score > localBestScore) { localBestScore = item.score; localBestFrameData = item; } });
      const scoresInFocus = currentFocusData.filter(d => d.score !== null && !d.error).map(d => d.score);
      const minL = scoresInFocus.length > 0 ? Math.min(...scoresInFocus) : 0; const maxL = scoresInFocus.length > 0 ? Math.max(...scoresInFocus) : 0; const rangeL = maxL - minL;
      setFocusedHeatmapData(currentFocusData.map(d => ({ ...d, isLocalBest: localBestFrameData ? d.frame === localBestFrameData.frame : false, localBestFrameData, normalizedScoreLocal: d.score === null || d.error ? 0.5 : (rangeL > TIME_EPSILON ? (d.score - minL) / rangeL : 0.5) })));
    } else if (!showFocusedHeatmap && focusedHeatmapData.length > 0) { setFocusedHeatmapData([]); }
  }, [showFocusedHeatmap, heatmapVersion, stableRefs.totalFrames, focusedHeatmapData.length]);

  // --- Global Heatmap Canvas Drawing Effect (same) ---
  useEffect(() => { /* ... same canvas drawing logic ... */
    ...
    if (stableRefs.overallBestFrame && stableRefs.overallBestFrame.frame > 0 && !stableRefs.overallBestFrame.error) { const bestFrame = stableRefs.overallBestFrame.frame; const xStart = ((bestFrame - 1) / localTotalFrames) * canvas.width; const frameWidthOnCanvas = (1 / localTotalFrames) * canvas.width; const rectWidth = Math.max(2, frameWidthOnCanvas); ctx.strokeStyle = 'gold'; ctx.lineWidth = 2; ctx.strokeRect(xStart - (rectWidth - frameWidthOnCanvas)/2 , 0, rectWidth, canvas.height); }
  }, [heatmapVersion, stableRefs.totalFrames, stableRefs.overallBestFrame, videoSrc, globalHeatmapCanvasWidth]);

  // Resize observer for global heatmap canvas container (same)
  useEffect(() => { /* ... same resize observer ... */
  ...
  const triggerPriorityAnalysis = useCallback((centerFrame) => { /* ... same ... */
    if (stableRefs.totalFrames === 0 || centerFrame === 0 || analysisTaskController.isGloballyCancelled) return; analysisTaskController.currentFocusedCenterFrame = centerFrame; setShowFocusedHeatmap(true); const initialFD = []; for (let i = -PRIORITY_ANALYSIS_RADIUS; i <= PRIORITY_ANALYSIS_RADIUS; i++) { const frame = centerFrame + i; if (frame >= 1 && frame <= stableRefs.totalFrames) { const eS = analysisTaskController.analyzedScores.get(frame); initialFD.push({ frame, score: eS && !eS.error ? eS.score:null, time: eS?eS.time:null, isCenter: frame===centerFrame, error: eS?!!eS.error:false, needsAnalysis: !eS||!!eS.error }); } } const scores = initialFD.filter(x=>x.score!==null && !x.error).map(x=>x.score); const minL=scores.length>0?Math.min(...scores):0; const maxL=scores.length>0?Math.max(...scores):0; const rangeL=maxL-minL; setFocusedHeatmapData(initialFD.map(d=>({...d, normalizedScoreLocal:d.score===null||d.error?0.5:(rangeL>TIME_EPSILON?(d.score-minL)/rangeL:0.5)}))); const newPT = []; for (let i = 0; i <= PRIORITY_ANALYSIS_RADIUS; i++) { if (i === 0) { if (!analysisTaskController.analyzedScores.has(centerFrame) || analysisTaskController.analyzedScores.get(centerFrame)?.error) newPT.push(centerFrame); } else { const pF = centerFrame - i; const nF = centerFrame + i; if (pF >= 1 && (!analysisTaskController.analyzedScores.has(pF) || analysisTaskController.analyzedScores.get(pF)?.error)) newPT.push(pF); if (nF <= stableRefs.totalFrames && (!analysisTaskController.analyzedScores.has(nF) || analysisTaskController.analyzedScores.get(nF)?.error)) newPT.push(nF); } } const uNT = [...new Set(newPT)]; analysisTaskController.priorityQueue = [...uNT, ...analysisTaskController.priorityQueue.filter(p => !uNT.includes(p))]; analysisTaskController.isRecursiveAnalysisPaused = true; if (uNT.length > 0) scheduleNextAnalysis();
  }, [stableRefs, setShowFocusedHeatmap, scheduleNextAnalysis]);

  const handleFileDrop = (file) => { /* ... same ... */
    if (file.type.startsWith('video/')) { if (analysisTaskController.nextScheduledAnalysisId) cancelAnimationFrame(analysisTaskController.nextScheduledAnalysisId); analysisTaskController.isGloballyCancelled = true; analysisTaskController.activeAnalysisProcesses = 0; workerTaskCallbacks.current.forEach(t => t.reject(new Error("New file"))); workerTaskCallbacks.current.clear(); workerBusyStates.current.fill(false); setVideoFile(file); const url = URL.createObjectURL(file); setVideoSrc(url); setCurrentTime(0); setDuration(0); setCurrentFrameNumber(0); setTotalFrames(0); setIsPlaying(false); setFrameRate(DEFAULT_FRAME_RATE); setShowFocusedHeatmap(false); setFocusedHeatmapData([]); setGlobalHeatmapCanvasWidth(0); return false; } message.error('Invalid video file.'); return Upload.LIST_IGNORE;
  };
  const onVisibleVideoMetadataLoaded = () => { /* ... same ... */ if (visibleVideoRef.current) setDuration(visibleVideoRef.current.duration); if (analysisVideoRef.current && videoSrc) analysisVideoRef.current.load(); };
  const handleTimeUpdate = () => { /* ... same ... */ if (visibleVideoRef.current && !analysisTaskController.isSeekingVideoLocked && !stableRefs.isUserInteracting && !visibleVideoRef.current.seeking) { const nT = visibleVideoRef.current.currentTime; if (Math.abs(stableRefs.currentTime - nT) > TIME_EPSILON * 2) setCurrentTime(nT); } };
  const handlePlayPause = () => { /* ... same ... */ if (visibleVideoRef.current && stableRefs.totalFrames > 0) { if (stableRefs.isPlaying) { visibleVideoRef.current.pause(); setIsPlaying(false); } else { analysisTaskController.isRecursiveAnalysisPaused = false; setShowFocusedHeatmap(false); visibleVideoRef.current.play().catch(e => console.warn("Play failed:", e)); setIsPlaying(true); } } };

  const userInitiatedSeekAction = useCallback(async (targetTime) => { /* ... same ... */
  ...
  const commonUserActionEnd = useCallback(() => { /* ... same ... */
  ...
  const handleSliderChange = (newFrameNumber) => { /* ... same ... */ if (stableRefs.totalFrames > 0) { if (!stableRefs.isUserInteracting) setIsUserInteracting(true); if (userInteractionTimeoutRef.current) clearTimeout(userInteractionTimeoutRef.current); const nT = frameToTime(newFrameNumber); if (visibleVideoRef.current) visibleVideoRef.current.currentTime = nT; setCurrentTime(nT); triggerPriorityAnalysis(newFrameNumber); } };
  const handleSliderAfterChange = (newFrameNumber) => { /* ... same ... */ const finalTime = frameToTime(newFrameNumber); userInitiatedSeekAction(finalTime).finally(commonUserActionEnd); };
  const handleHeatmapSegmentClick = (targetFrame) => { /* ... same null check ... */ if (targetFrame === null || targetFrame === undefined || targetFrame <= 0 || targetFrame > stableRefs.totalFrames) return; setIsUserInteracting(true); const time = frameToTime(targetFrame); userInitiatedSeekAction(time).finally(commonUserActionEnd); };
  const handleGlobalHeatmapCanvasClick = (event) => { /* ... same ... */ if (!globalHeatmapCanvasRef.current || stableRefs.totalFrames === 0 || stableRefs.isUserInteracting) return; const canvas = globalHeatmapCanvasRef.current; const rect = canvas.getBoundingClientRect(); const x = event.clientX - rect.left; const clickedFrame = Math.floor((x / canvas.width) * stableRefs.totalFrames) + 1; const targetFrame = Math.max(1, Math.min(clickedFrame, stableRefs.totalFrames)); handleHeatmapSegmentClick(targetFrame); };
  const handleStepFrame = (direction) => { /* ... same ... */ setIsUserInteracting(true); if (stableRefs.totalFrames > 0) { let tF = stableRefs.currentFrameNumber + direction; tF = Math.max(1, Math.min(tF, stableRefs.totalFrames)); if (tF === stableRefs.currentFrameNumber && Math.abs(frameToTime(stableRefs.currentFrameNumber) - frameToTime(tF)) < TIME_EPSILON) { setIsUserInteracting(false); if (analysisTaskController.priorityQueue.length === 0) analysisTaskController.isRecursiveAnalysisPaused = false; return; } userInitiatedSeekAction(frameToTime(tF)).finally(commonUserActionEnd); } else { setIsUserInteracting(false); if (analysisTaskController.priorityQueue.length === 0) analysisTaskController.isRecursiveAnalysisPaused = false; } };
  const formatFrameForFilename = (frameNum) => `frame_${String(frameNum).padStart(6, '0')}`;
  const handleDownloadFrame = () => { /* ... same ... */ if (!visibleVideoRef.current || !downloadCanvasRef.current || currentFrameNumber === 0) return; const v = visibleVideoRef.current; const c = downloadCanvasRef.current; c.width=v.videoWidth; c.height=v.videoHeight; const ctx = c.getContext('2d'); ctx.drawImage(v,0,0,c.width,c.height); const dU=c.toDataURL('image/png'); const a=document.createElement('a');a.href=dU;const fNB=videoFile?.name.replace(/[^a-zA-Z0-9_.-]/g,'_')||'video'; a.download=`${fNB}_${formatFrameForFilename(currentFrameNumber)}.png`;document.body.appendChild(a);a.click();document.body.removeChild(a); message.success(`F${currentFrameNumber} downloaded!`); };

  // --- NEW Button Handlers ---
  const handleGoToOverallBestFrame = () => {
    if (overallBestFrame && overallBestFrame.frame > 0) {
      handleHeatmapSegmentClick(overallBestFrame.frame);
    } else {
      message.info("Overall best frame not determined yet.");
    }
  };
  ...
  const handleGoToLocalBestFrame = () => {
    if (showFocusedHeatmap && focusedHeatmapData.length > 0) {
      ...
      if (localBest && localBest.frame > 0) {
        handleHeatmapSegmentClick(localBest.frame);
      } else {
        message.info("Local best frame not determined in current focused view, or view not active.");
      }
    } else {
  ...
  // --- Calculate style for the focused range indicator on the main timeline (same) ---
  const focusedRangeIndicatorStyle = useMemo(() => { /* ... same ... */
    if (!showFocusedHeatmap || focusedHeatmapData.length === 0 || !stableRefs.totalFrames) return { display: 'none' }; const framesInFocus = focusedHeatmapData.map(d => d.frame).filter(f => f !== undefined && f !== null); if (framesInFocus.length === 0) return { display: 'none' }; const minFrame = Math.min(...framesInFocus); const maxFrame = Math.max(...framesInFocus); if (minFrame < 1 || maxFrame > stableRefs.totalFrames || minFrame > maxFrame) return { display: 'none' }; const leftPercentage = ((minFrame - 1) / stableRefs.totalFrames) * 100; const widthPercentage = ((maxFrame - minFrame + 1) / stableRefs.totalFrames) * 100; return { display: 'block', position: 'absolute', top: '0px', left: `${Math.max(0, Math.min(100, leftPercentage))}%`, width: `${Math.max(0, Math.min(100, widthPercentage))}%`, height: '100%', };
  }, [showFocusedHeatmap, focusedHeatmapData, stableRefs.totalFrames]);
  ...
  return (
    <Layout className="app-layout">
        ...
        {videoSrc && (
          <div className="video-player-container">
            <video key={videoSrc + "-visible"} ref={visibleVideoRef} src={videoSrc} className="video-element"
              ...
              onPause={() => setIsPlaying(false)} onClick={handlePlayPause} controls preload="metadata"
            />
              ...
              <div className="controls-wrapper">
                  ...
                  {showFocusedHeatmap && focusedHeatmapData.length > 0 && (
                    <div className="focused-heatmap-wrapper">
                        ...
                        {focusedHeatmapData.map(data => {
                            ...
                            if (data.error) { segmentColor = '#a0a0a0'; }
                            else if (data.score !== null) { const hue = Math.max(0, Math.min(1, data.normalizedScoreLocal)) * 120; segmentColor = `hsl(${hue}, 70%, 50%)`; }
                            else if (data.needsAnalysis) { segmentColor = '#ccc'; }
                            const title = `F${data.frame}${data.error ? ' (Error)' : (data.score !== null ? `, Score: ${data.score.toFixed(3)}` : (data.needsAnalysis || !analysisTaskController.analyzedScores.has(data.frame) || analysisTaskController.analyzedScores.get(data.frame)?.error ? ' (Analyzing...)' : ' (Pending)'))}`;
                            return (
                ...
                <Space wrap className="action-buttons-group">
                  ...
                  <Tooltip title="Go to Overall Best Frame">
                    <Button icon={<StarOutlined />} onClick={handleGoToOverallBestFrame} disabled={!overallBestFrame || totalFrames === 0 || isUserInteracting} />
                  </Tooltip>
                  <Tooltip title="Go to Local Best Frame (in focused view)">
                    <Button icon={<EyeOutlined />} onClick={handleGoToLocalBestFrame} disabled={!showFocusedHeatmap || focusedHeatmapData.every(d => !d.isLocalBest) || totalFrames === 0 || isUserInteracting} />
                  </Tooltip>
      ...
      <Footer style={{ textAlign: 'center', background: '#f0f2f5' }}><Paragraph type="secondary">Sharp Frame Picker ©{new Date().getFullYear()}. (Assumes {DEFAULT_FRAME_RATE} FPS)</Paragraph></Footer>
    </Layout>
  );
}
export default App;